...
// Statcode format is [logToFile, prefixText"].
errorBody.statCodes[0] = [true, "     ERROR ".bgWhite.black];
errorBody.statCodes[1] = [false, "     DEBUG ".bgBlue.white];
...
{	
	if(frames[type] == undefined)
	{
		...
		{
			prefix = selectedFrame.statCodes[code][1];
			
			if(selectedFrame.statCodes[code][0])
			{
				global.errorDump.write(text + '\n');
			}
		}
		...
		{
			frames[type].log(prefix + lines[l]);
		}
