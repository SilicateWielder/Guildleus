...
function getProcessId(pid, dataBundle) 
{
	...
	else
	{
		...
		for(let candidate = 0; candidate < similarTasks.length; candidate++)
		{
			let pid = similarTasks[candidate];
			let task = processTable[pid];
			
			if(task.channel == dataBundle.message.channel.id && task.command == dataBundle.cmdName)
			{
				return (pid);
			}
		}
...
exports.create = function(dataBundle, runCondition, endCondition) {
	...
	let newProcess = {
		...
		'command': issuer, // Command that issued this process.
	...
	processListing[newProcess.id] = [newProcess.id, newProcess.name, newProcess.command, newProcess.channel, runString, endString];
