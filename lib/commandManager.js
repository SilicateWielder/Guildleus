		...
		if(synonyms != undefined)
		{
			for(let s = 0; s < synonyms.length; s++)
			{
				if(commandRef[synonyms[s]] == undefined)
				{
					...
					displayManager.log('main', `	Adding key '${synonyms[s]}' for command '${keys[c]}'...`);
				} else {
					displayManager.log('main', `	key '${synonyms[s]}' already exists, skipping...`);
				}
			}
		}
	}
...
exports.init = function()
{
	...
	for(cid = 0; cid < commandKeys; cid++);
	{
		let key = commandKeys[cid];
	...
	if(messageStart == config.prefix)
	{
		...
		for(let c = 0; c < queue.length; c++)
		{
			...
			if(commandRef[command.key] != undefined) {
				...
				commandTable[newKey].run(command.args, dataBundle);
			}
		}
	}
}